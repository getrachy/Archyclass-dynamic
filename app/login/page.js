  ...
  const handleAuth = async (e) => {
    ...
    try {
      if (view === 'login') {
        ...
        if (profile?.class) {
          router.push('/dashboard')  // Old user → skip onboarding
        } else {
          router.push('/onboarding') // New user → pick class
        }
      } 
      ...
      if (view === 'signup') {
        const { error } = await supabase.auth.signUp({ 
          ...
          password,
          options: { 
            emailRedirectTo: `${window.location.origin}/onboarding` // Changed this
          }
        })
        if (error) throw error
